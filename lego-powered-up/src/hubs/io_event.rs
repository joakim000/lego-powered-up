...
pub async fn io_event_handler(
    ...
    loop {
        tokio::select! {
            ...
            Some(data) = stream.next() => {      //  while let Some(data) = stream.next().await {
                                        ...
                                        PortInformationType::ModeInfo{capabilities, mode_count, input_modes, output_modes} => {
                                            {
                                                ...
                                                for mode_id in 0..mode_count {
                                                    ...
                                                    // hub.req_mode_info(port_id, mode_id, ModeInformationType::MotorBias).await.await?;          // Returns errorcode CommandNotRecognized on all devices I've tested
